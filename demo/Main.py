...
class LR:
    ...
    def predict(self):
        self.loadTestData()
        self.labels_predict = (self.sigmod(np.dot(self.feats_test, self.weight) + self.b)+0.5).astype(np.int)
    ...
    def train(self):
        ...
        for j in range(self.max_iters):
            dataIndex=list(range(recNum))
            for i in range(recNum):
                randIndex = int(random.uniform(0,len(dataIndex)))
                gradient = self.labels[randIndex] - self.sigmod(np.dot(self.feats[randIndex], self.weight) + self.b)
                self.b += self.lr * gradient
                self.weight += (self.lr * gradient * self.feats[randIndex]).reshape(-1)
